...
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE FFTT( X, Y, N, M, ITYPE, WR, WI )
c
c  This is a very simple Cooley-Tukey Radix-2 DIF Complex FFT
c  with table lookup of sines and cosines.
c 
c  The data sequence is of size N = 2**M.
c  X and Y contain the real and imaginary parts of the data.
c
...
c  ITYPE .eq. -1 for backward transform
c
c  WR and WI are determined by the table lookup routine below.
c  Both arrays WR and WI have size N=2**M.  The subroutine INIT
c  must be called prior to calling this routine.
c
c  The forward transform computes
c     Z(k) = sum_{j=0}^{N-1} z(j)*exp(-2ijk*pi/N)
c
...
c
c
c-------table lookup routine-------------------------------c
c
c
      SUBROUTINE INIT( N, WR, WI )
c ... Scalar arguments ...
      INTEGER  N
c ... Array arguments ...
      REAL  WR(*), WI(*)
