...
c      PARAMETER     (NPRV = 4*8192, NPR = 1025, ZERO = +0.0D+0)
...
c 
      ...
      REAL*8        UAUX, VAUX, CS, SN, DEM, DE, PI
c
...
c            SGAUX = (2.0D0/PI*ATAN((TP - T)/TT) + 1.0D+0)/2.0D+0
            IF(J.EQ.1)write(11,*)t,theta(t,tp,tf), sgaux
            IF(DEM.EQ.0)THEN
               CS = 1.0D+0
               SN = 0.0D+0
            ELSE
            ...
            ENDIF               
            U(I) = (UAUX*CS - VAUX*SN)*SGAUX*THETA(T,TP,TF)
            V(I) = (VAUX*CS + UAUX*SN)*SGAUX*THETA(T,TP,TF)
c            U(IAUX) = 0d0
c            V(IAUX) = 0d0
