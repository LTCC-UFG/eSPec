      SUBROUTINE PSOD(ABSORB, DIM, PRTCRL, TPABSOR, INFO, LC, MP, MPI, 
...
cdel      CHARACTER*(*) 
      INTEGER       NP(*)
      REAL*8        SHM(*), U1(*), U2(*), V1(*), V2(*), XI(*)
      REAL*8        XF(*), VPOT(*), VABC(*), WORK(*), VAR(*)
      REAL*8        LNZVC(MXDCT,*)
c     **
*     ..
*     Purpose
...
c     FNI = FCORREL(N, U2, V2) - FCORREL(N, V2, U2)
      FNT = FNR
      F0 = FNT
c
     ...
     &        '*|C(t)|^2*', '*|C|*'
      ...
      ENDIF
c     .. First half step in the propagation towards T+DT/2 (Taylor)
      CALL PSODAUX(ABSORB, TPABSOR, DIM, N, CST/4D0, NP, SHM, U1, U2, 
     &        V1, V2, VPOT, VABC, WORK, VAR)
c     .. Second half step in the propagation towards T+DT (SOD)     
...
c      FNI = FCORREL(N, U2, V2) - FCORREL(N, V2, U2)
      FNT = FNR
      IF(ABSORB)THEN
         IF(ABS((ET-E0)/E0)*1.0D+2.GT.TOL)INFO = ZERO
      ELSE
         IF(ABS((ET-E0)/E0)*1.0D+2.GT.TOL 
     &        .OR. ABS((FNT-F0)/F0)*1.0D+2.GT.TOL)INFO = ONE
      ENDIF
