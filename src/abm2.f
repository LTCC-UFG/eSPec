...
c     ==================
      SUBROUTINE PABMAUX1(ABSORB, TPABSOR, DIM, N, CST, THREECST, 
...
c     
      ...
      ELSE
         IF(CSGAMMA1.EQ.ZERO .AND. CSGAMMA2.EQ.ZERO)THEN
            DO I=1,N,1  
               UAUX  = U2(I)
               U2(I) = UAUX + 0.0D0*THREECST*WORK(I) - 0.0D0*CST*HU1(I) 
               HU1(I) = WORK(I)
               U1(I) = UAUX
            ENDDO
         ELSE
            DO I=1,N,1  
               UAUX  = U2(I)
               U2(I) = UAUX + 0.0D0*THREECST*WORK(I) - CSGAMMA1*UAUX
     &              - 0.0D0*CST*HU1(I) + CSGAMMA2*U1(I)
               HU1(I) = WORK(I)
               U1(I) = UAUX
            ENDDO
         ENDIF
...
c   
      ...
      ELSE
         IF(CSGAMMA1.EQ.ZERO .AND. CSGAMMA2.EQ.ZERO)THEN
            DO I=1,N,1
               VAUX = V2(I)
               V2(I) = VAUX - 0.0D0*THREECST*WORK(I) + 0.0D0*CST*HV1(I) 
               HV1(I) = WORK(I)
               V1(I) = VAUX
            ENDDO   
         ELSE
            DO I=1,N,1
               VAUX = V2(I)
               V2(I) = VAUX - 0.0D0*THREECST*WORK(I) - CSGAMMA1*VAUX 
     &              + 0.0D0*CST*HV1(I) + CSGAMMA2*V1(I)
               HV1(I) = WORK(I)
               V1(I) = VAUX
            ENDDO   
         ENDIF
