     ...
     &     rK, rA, X0, VOI, VAR,PRTABS)
      IMPLICIT NONE
c     **
c     ** Scalar arguments
      LOGICAL       ABSORB, CHANGE, PRTEGVC, PRTPOT, PRTVEFF, PRTEIGVC2
      LOGICAL       PRTPULS, PRTDIPL, LHWHM, FSTOP, SHIFTP, PRTABS
...
C---------Vinicius 27/10/2011-----------------------------------
C---------------------------------------------------------------  
         ELSEIF(READAUX(1:16).EQ.'*PRINT_ABSORBING')THEN   
            READ(1,'(A)',END=999,ERR=999)READAUX
            IF(READAUX(1:4).EQ.'.YES' .OR. READAUX(1:3).EQ.'.ON')THEN
               PRTABS = .TRUE.
            ELSE
               PRTABS = .FALSE.
            ENDIF
