...
c
      ...
      PARAMETER     (NPRV = 4*8192, NPR = 2*1025, MNPR = NPRV*NPR)
...
c 
      ...
      REAL*8        DT, TI, TF, TS, XMAX, UMAX, XAV2
c
...
c
     ...
     &        '" cannot be opened <<<>>>'
      ...
      UMAX = ZERO
      DO J=1,NPR,1
         READ(2,*, END=10,ERR=999)XL(J), UM(I,J), VM(I,J)
      ENDDO
      WRITE(1,*)XMAX,DT
 10   CONTINUE
c
...
c
      XAV2 = ZERO
      DO J=1,NJ,1
         XAV2 = XAV2 + UM(I,J)*XL(J)*XL(J)*UM(I,J) 
     &        + VM(I,J)*XL(J)*XL(J)*VM(I,J)
      ENDDO
      WRITE(1,*)DT, XAV2
c
      I = 2
